...
# Жёсткие границы, дальше которых пост не пустим. Верхняя — предел Telegram
# на одно сообщение. Границы из настроек держат модель, а редактору иногда
# нужен пост длиннее — на такой случай остаётся только предел площадки.
#
# Подпись к альбому у Telegram короче, 1024 символа. Пост длиннее уходит
# отдельным сообщением под фотографиями — этим занимается telegram/client.py.
POST_CHARS_FLOOR = 200
POST_CHARS_CEILING = 4096
...
class LlmSettings(Base):
    ...
    # поэтому запоминаем сам отказ и показываем его в настройках.
    last_ok_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    last_error: Mapped[str | None] = mapped_column(Text)
    last_error_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    out_of_money: Mapped[bool] = mapped_column(Boolean, default=False)
